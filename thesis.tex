...
SAWs are generated by interdigitated transducers, or IDTs. IDTs consist of pairs of interdigitated electrodes, or "fingers", which convert an RF signal into surface acoustic waves, or surface acoustic waves into an RF signal. Figure \ref{SAW diagram} shows a schematic of an IDT. An RF signal is applied such that adjacent fingers have opposite polarity. The wavelength $lambda$ of the generated SAW spans two fingers of the IDT. Because of this, one of the defining parameters of an IDT is the number of finger pairs $N_p$. 

The electrical response of an IDT can be approximated by assuming that each IDT finger is an infinitely thin point source, spaced by $\lambda/2$. This is known as the "point source model" or "delta function model". The frequency response, or transfer function, of a single IDT in the delta function model can be approximated as 
...
It is desirable to minimize the loss between applied RF power and output SAW power. For example, in the experiments described in Ch. \ref{AE charge pumping paper}, a higher output SAW power results in a larger acoustoelectric signal. Most RF signal generators are designed with \SI{50}{\ohm} input impedance. If I power my IDT of impedance $Z_{IDT}$, the ratio of input RF power $P_{RF}$ to resultant SAW power $P_{SAW}$ (or, transmission coefficient) is
...
\begin{align}
    ...
    &= G_0 \left(\frac{\sin X}{X}\right)^2 + i \left(\frac{\sin (2X) - 2X}{2X^2} + 2\pi f C_{IDT}\right), \label{IDT admittance}
\end{align}
...
Before using an interdigitated transducer in an acoustoelectric device (Ch. \ref{AE charge pumping paper}), I need to verify its performance. In Sec. \ref{SAW device design}, I outline the expected frequency response and impedance of an IDT based on its geometry. In this section, I discuss measuring the frequency response and impedance of IDTs to verify their performance. 

Figure \ref{SAW device optical image with measurements} shows an optical microscope image of an IDT that I fabricated on LiNbO\textsubscript{3} using photolithography (see App. \ref{photolithography recipe}) and metallization of $5/\SI{25}{\nano\meter}$ Cr/Au.
...
\begin{figure}
    ...
    \label{S-parameters}
\end{figure}
The S-parameter data shown in Fig. \ref{S-parameters} (b) and (c) have the expected behavior. First, there is a drop in reflected power at $f_0$ (Fig. \ref{S-parameters} (b)), which corresponds to the increase in transmitted power (Fig. \ref{S-parameters} (c)). Away from $f_0$, the S21 and S12 data match the expected frequency response predicted by the model discussed in Sec. \ref{SAW device design} (Eq. \ref{Response of IDT}).

Figure \ref{Z11 plot} shows the impedance of IDT 1 in comparison to the model given by Eq. \ref{IDT admittance}. The measured impedance at $f_0$ is \SI{290}{ohm}, while the model predicts an impedance of \SI{170}{ohm}. This is most likely because the model given by Eq. \ref{IDT admittance} does not consider reflections from each IDT finger. This IDT has $N_p = 20$, and each finger reflects around $0.1-2 \%$ of the IDT signal. 

From Eq. \ref{transmission coefficient}, $\approx 70\%$ of the input RF power is reflected back from the IDT. This could be improved; however, this IDT has similar performance to 

%TODO: Fix ticks on all of these plots
\begin{figure}
