...
\end{quote}
Or, put more simply, quantum materials are those "whose electronic properties cannot be understood with concepts from contemporary condensed-matter textbooks" \cite{orenstein_ultrafast_2012}. 
...
Carbon nanotubes (CNTs) are essentially rolled-up sheets of graphene, forming a 1D tube-like structure. Similar to graphene, CNTs also exhibit a valley degree of freedom. However, the physics of valleys in CNTs is even more intriguing due to their 1D nature. Depending on how the sheet of graphene is rolled up, nanotubes can have widely different properties. The confinement of electrons resulting in discrete subbands in the band structure. Each subband corresponds to a different “valley”, and the valley index can be used as a pseudospin, similar to the case of graphene.

Senger 2018 paper talking about subbands and valley
...
Topological phases of matter are defined by global properties. The information that defines the properties of a topological material

It is predicted that in multi-valley 2D semiconductors under the influence of a SAW, a "valley acoustoelectric Hall effect" arises, where 


"The concept of the Berry phase [27] underlies and unifies diverse aspects of solid-state physics, drastically affecting the transport of particles and resulting in such intriguing phenomena as the anomalous [28] and quantum [29] Hall effects, emergence of topological and superconducting phases [30], charge pumping [31], and anomalous thermoelectric transport [32], among other [33]. By and large, electrons in a crystal behave similarly to free particles with just the free-electron mass replaced by an effective one due to the formation of energy bands. A nontrivial Berry phase also reveals itself in Dirac materials like graphene [34], and very recently it has come into play in other two dimensional materials [35], possessing similar symmetry properties."

\cite{kalameitsev_valley_2019}
...
Here, I briefly summarize the origin of geometric phase under an adiabatic evolution, following Ref. \cite[Ch. 5]{sakurai_modern_1985}. The time evolution of a quantum system is described by the time-dependent Schrödinger equation, which is given by
...
\end{equation}
The second term of Eq. \ref{c-dot eqn} tells us that, in a system with a time-dependent Hamiltonian, states $\ket{\Psi_n}$ will evolve to states $\ket{\Psi_m}$ because of the time dependence of the Hamiltonian, and that this evolution will take place at a frequency $1/\tau$, defined as 
...
\end{equation}
where $S$ is a closed surface in the parameter space (in the case of the Thouless pump, the 2D space of momentum and time), and $\vectorbold{\Omega}$ is the Berry curvature. The Berry curvature is analogous to the Gaussian curvature $K$, which measures the local curvature of a closed surface. For a 2D closed surface with Gaussian curvature $K$, 

Use Gauss Bonnet to connect to Chern number

Now, consider the Thouless pump system, where the Hamiltonian is varied adiabatically by a potential with temporal period $T$ and spatial period $a$. 
...
From the Berry curvature, we can finally arrive at the topological invariant, the \textit{Chern number}. The Chern number 
...
A quantized charge pump is a device that transports electrons in precise, quantized units. Unlike conventional DC transport, where electrons flow stochastically in response to an applied voltage bias, quantized charge pumps operate at a constant frequency $f$, transporting a unit of charge $Ne$ per cycle (where $N$ is an integer), resulting in a DC current of magnitude $Nef$ that arrives in quantized packets of charge $Ne$. Such devices are of great interest to metrologists, where practical sources of highly-quantized current could help define a better standard of current \cite{scherer_singleelectron_2019}. Many designs for quantized charge pumps have been demonstrated, with the best quantum pumps today being based on controlling tunneling to- and from quantum dots. However, charge pumps based on tunneling face fundamental limits to their accuracy. A topologically-protected charge pump could overcome these fundamental limits.

Topological charge pumping is the transport of charge without external magnetic or electric fields arising from a cyclic adiabatic evolution of the system's Hamiltonian. This phenomenon is often referred to as Thouless pumping, after D.J. Thouless, the first to propose such a phenomenon \cite{thouless_quantization_1983}. Thouless found that, in a 1D system under the influence of a spatially- and temporally periodic potential, the amount of charge \footnote{More accurately, Thouless's prediction was about \textit{particle transport} in general, which includes electrons.} transported per cycle is an integer, if the system begins and ends in an eigenstate of the Hamiltonian (which is a requirement of the adiabatic theorem, discussed below). Furthermore, the quantization of charge transport in the Thouless pump is a \textit{topological invariant}, which means that it only depends on the topology of the band structure of the system, and is invariant to variations in the exact cycle, like the temporal period. A topologically protected pump would be robust to perturbations from its environment, allowing extremely precise quantized current \cite{niu_towards_1990}. But, what is "topological" about the Thouless pump? It turns out that the pumped charge depends only on the geometric phase acquired by the system during one cycle, which can be shown to be a topological invariant under an adiabatic perturbation. 
...
Though the Thouless pump has been realized in ultracold atoms \cite{citro_thouless_2023}, a charge Thouless pump suitable to serve as a source of quantized current has not yet been realized experimentally. A practical source of topologically protected quantized current is of great interest to the fields of metrology \cite{pekola_single-electron_2013}, where topologically protected charge pumping could enable higher-accuracy quantized charge pumps, and quantum computing \cite{das_controlled_2006}, where a Thouless pump could generate and control the flow of spin-entangled quantum states with minimal decoherence. Talyanskii et al. proposed that a carbon nanotube coupled to a surface acoustic wave is an ideal system to create a charge Thouless pump \cite{talyanskii_quantized_2001}. 
